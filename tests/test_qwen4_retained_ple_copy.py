...
@pytest.fixture(params=["diagnostic", "production"])
def copy_bits(request):
    if request.param == "production":
        return copy_history_bits
    return lambda value: copy_ple_bits(value, array_module=mx, numpy_module=np)


@pytest.mark.parametrize("dtype", [mx.bfloat16, mx.float16])
def test_detachment_preserves_all_16bit_patterns_and_shape(dtype, copy_bits):
...
def test_detachment_preserves_noncontiguous_tail_view(copy_bits):
...
def test_wrong_dtype_is_not_silently_cast(copy_bits):
    with pytest.raises(ValueError, match="16-bit"):
        copy_bits(mx.array([1], dtype=mx.float32))
...
def test_real_deltanet_conv_continuation_is_bit_exact_after_detach(
        dtype, prefix_length, suffix_length, copy_bits):
